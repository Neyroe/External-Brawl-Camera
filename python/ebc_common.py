...
DISPLAY_HURTBOX = 0x4E0D5F # 0 = default, 1 = hurtbox On, 2 = hurtbox On + model Off
DISPLAY_CHARMODEL = 0x6732F3 # 2 default, 0 for disable   WIP could be better
DISPLAY_STAGE_COLLISION = 0x4E0DE3 # Using debug menu, 0 = default, 1 = collision On, 2 = Stage Hide + collision On 
...
def find_RSBE01():
    handle = EMU.process_handle
    byte_pattern = bytes.fromhex("52 53 42 45 30 31 00 01") #RSBE01 byte array
    found = pattern_scan_all(handle, byte_pattern)
    print(f'RSBE01 have been found') if found else print(f"RSBE01 hasn't been found")        
    return found

def find_dolphin_funcs(byte_pattern):
    handle = EMU.process_handle
    module = pymem.process.module_from_name(EMU.process_handle, "Dolphin.exe")
